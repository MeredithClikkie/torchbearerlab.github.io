...
stop_words = [
    ...
    "not", "no", "yes", "so", "can", "just", "dont", "yeah", "let", "im", "ah", "want", "like", "ive", "all", "ooh", "ill", "even", "forforforforforfor", "yeahyeahyeahyeah", "daisychained" "thats", "there", "oh", "hell", "wasnt", "when", "shouldnt", "would", "than", "cant", "youre"
]
...
lyrics = """Can't believe you're gone for good
...
plt.title('Cottonwood Frequent Words')
plt.tight_layout()

# Save or display
plt.savefig('cottonwood_bar.png')
# %% [markdown]
# ### Cottonwood Lollipop
# %%
...
plt.title('Cottonwood Lollipop', fontsize=14, fontweight='bold')
...
# 4. Save or display
plt.savefig('cottonwood_lollipop.png')
plt.show()
# %% [markdown]
# ### Cottonwood Bubble Chart
# %%
# 1. Prepare your data (from your existing code)
df_counts = pd.DataFrame(counts.most_common(15), columns=['Word', 'Count'])
...
# 4. Styling
plt.title('Cottonwood Treemap', fontsize=16)
plt.axis('off') # Treemaps don't need axes

plt.tight_layout()
plt.savefig('cottonwood_treemap.png')
plt.show()
# %% [markdown]
# ### Cottonwood Word Cloud
# %%
...
plt.title("Cottonwood Radial", va='bottom')
plt.show()
# %% [markdown]
# ### Cottonwood Funnel Chart
# %%
df_counts = pd.DataFrame(counts.most_common(15), columns=['Word', 'Count'])

fig = px.funnel(df_counts, x='Count', y='Word',
                title='Cottonwood Funnel',
                color = 'Word',
