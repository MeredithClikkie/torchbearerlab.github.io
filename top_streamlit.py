...
empty_songs = [i for i, text in enumerate(processed_songs) if len(text) == 0]
if empty_songs:
...
df_combined.head()
# %%
df_topics['track_name'] = df_topics['Song_Name']
df_topics.head()
# %%
# Merge df_topics into df_combined based on a common column (e.g., 'track_name')
df_plot = pd.merge(df_combined, df_topics, on='track_name')
# %%
import matplotlib.pyplot as plt
...
wordcloud.to_file("each_album_wordcloud.png")
...
st.title("|-/ Twenty One Pilots: Lyric Analytics")

# Sidebar Filter
selected_album = st.sidebar.selectbox("Select Album", df_plot['album_name'].unique())
filtered_df = df_plot[df_plot['album_name'] == selected_album]

# Visualizing Faith vs Hope
st.subheader(f"Faith vs. Hope in {selected_album}")
fig = px.scatter(filtered_df, x="Faith_Count", y="Hope_Count",
                 size="Word_Count", hover_name="track_name", color="Topic_Label")
st.plotly_chart(fig)

# Show Lyrics for a selected track
track = st.selectbox("Select a track to read lyrics:", filtered_df['track_name'])
st.text_area("Lyrics", df[df['track_name'] == track]['Lyrics'].values[0], height=200)
# %% [markdown]
# import streamlit as st
# import plotly.express as px
# 
